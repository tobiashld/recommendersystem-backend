...
[2]# Get complete Dataset
df = pd.read_csv('refactored_data_complete.csv', names = ['User_Id', 'Rating','Movie_Id'])

[3]# Drop Ratings under 5
df = df.drop(df[df.Rating < 5].index)

[4]# Exclude Ratings from users who have rated under 20 movies with 5
min_user_ratings = 19
...
[5]# Get 1.000 random Users matching the inclusion criteria
user_ids = df_filterd['User_Id'].unique().tolist()
random_user_ids = random.choices(user_ids, k = 1000)

[6]# Get part of the dataframe including those users and evaluate value counts
df_sample = df.loc[df['User_Id'].isin(random_user_ids)]
df_sample['Rating'].value_counts()

[7]# Refactor sample and extract it into a dict of the following form:
